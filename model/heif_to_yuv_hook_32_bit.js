...
function sava_input_buff(lib) {
	b(lib.add(0x7B_A0 + 1), c => {
		...
		dump(c.r1); // Byte!!!
		writeFile(Memory.readByteArray(c.r1, Number.parseInt(c.r2)), '/sdcard/prepare_to_yuv.log');
	});
}
...
function save_output_buff(lib) {
	...
	b(lib.add(offset + 1), ctx => {
		...
		send('0x0014->' + Number.parseInt(0x00_14));
		out_buffer = ctx.r1;
	}, ctx => {
		if (Number.parseInt(height) < 0x60) {
			return;
		...
		size = ctx.r0;
		send('size->' + size);
		writeFile(Memory.readByteArray(out_buffer, Number.parseInt(size)), '/sdcard/out_data_yuv.yuv');
	});
}
